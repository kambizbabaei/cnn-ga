...
class EvolveCNN(object):
    ...
    def initialize_population(self):
        # ADDED: begin_evolution is called to flag that GA is running
    ...
    def fitness_evaluate(self):
        # Evaluate the individuals’ fitness by training them (see evaluate.py).
    ...
    def environment_selection(self):
        ...
        # We use Roulette selection to pick the next generation
    ...
    def do_work(self, max_gen):
        Log.info('*' * 25)
        # Step 1: Check if we are continuing from a saved state or starting fresh
        if StatusUpdateTool.is_evolution_running():
            ...
            if gen_no is not None:
                ...
                raise ValueError('The running flag is set, but no population data is found')
        else:
            gen_no = 0
            Log.info('Initialize...')
