...
class FitnessEvaluate(object):
    ...
    def evaluate(self):
        ...
        so, a while-loop needs to check whether all GPUs are available.
        Only when all GPUs are free can we conclude
        "the evaluation for all individuals in this generation" is finished.
        """
        ...
        """
        After all the individuals in this generation have been evaluated,
        retrieve the fitness from after_XX.txt for any that are still -1.0.
        """
        if has_evaluated_offspring:
            ...
            for indi in self.individuals:
                if indi.acc == -1:
                    if indi.id not in fitness_map:
                        self.log.warn(
                            'The individuals have been evaluated, but the records are not correct, '
